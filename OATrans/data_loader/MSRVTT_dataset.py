...
# from base.base_dataset_global_local import TextObjectVideoDataset
# from base.base_dataset_region_mem import TextObjectVideoDataset
import pandas as pd
import os
import json
import numpy as np
...
class MSRVTT(TextObjectVideoDataset):
    ...
    def _get_object_path(self, sample):
        ...
        Returns:
            abs path
        """
        # real_path = os.path.join(sample.name, '{}.npz'.format(index))
        real_path = sample.name
        full_object_fp = os.path.join(self.object_dir, sample.name)
