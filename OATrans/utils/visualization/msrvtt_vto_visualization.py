...
full_json = "MSRVTT/annotation/MSR_VTT.json"
data_source = "MSRVTT/videos/all"
feat_source = "MSRVTT/region_features"
...
for row in data['annotations']:
    ...
    ret, img = cap.read()
    feat_path = os.path.join(feat_source, row['image_id'], '1.npz')
    ...
    wrapped_text = textwrap.wrap(row['caption'], width=35)
    x, y = 10, 40
    font_size = 1
    font_thickness = 2
    ...
    # cv2.imshow(concat)
    out_file = os.path.join(output, row['image_id'] + '_' + str(row['id']) + '.png')
    print("hello world")
