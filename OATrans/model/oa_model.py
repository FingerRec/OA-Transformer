...
import torch
from OATrans.model.video_transformer import SpaceTimeTransformer
...
class FrozenInTime(BaseModel):
    ...
    def forward(self, data, aug=False, return_embeds=True):
        ...
        video_embeddings = self.compute_video(video_data, aug=aug)
        if return_embeds:
            return text_embeddings, video_embeddings
        return sim_matrix(text_embeddings, video_embeddings)

    def compute_text(self, text_data, pad=False):
        # load from prezrained dir
        # print(self.text_params['model'].split('/'))
        ...
        # text_embeddings_all = self.txt_proj_2(text_embeddings_all)
        return text_embeddings #, text_embeddings_all

    def compute_object(self, object_data):
        object_embeddings = self.object_model(object_data)
        return object_embeddings

    def compute_video(self, video_data, aug=False):
        video_embeddings, patch_embeddings = self.video_model(video_data)
        video_embeddings_proj = self.vid_proj(video_embeddings)
        m = nn.Sigmoid()
        return video_embeddings_proj
