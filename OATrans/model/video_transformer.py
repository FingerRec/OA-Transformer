...
class SpaceTimeTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.num_frames = num_frames
        # print("num frames is: ".format(self.num_frames))
        self.embed_dim = embed_dim
    ...
    def forward_features(self, x, aug=False):
        # print(x.size(), self.num_frames)
        b, curr_frames, channels, _, _ = x.shape
        x = self.patch_embed(x)
        x = x.flatten(2).transpose(2, 1)
        x = x.reshape(b, -1, self.patch_embed.embed_dim)
        if aug:
            x = patch_consistent_augmentation(x, v=0)
        ...
        for blk in self.blocks:
            x = blk(x, self.einops_from_space, self.einops_to_space, self.einops_from_time,
                    self.einops_to_time,
                    time_n=n, space_f=f)
        # print(x.size()) # b x 197 x 768
        # # v1: original implementation
        # x_n = self.norm(x)[:, 0] # + self.max_pool(self.norm(x)[:, 1:].permute(0, 2, 1)).squeeze(2)
        # # x_n = 1 / 2 * self.norm(x)[:, 0] + 1 / 2 * torch.mean(self.norm(x)[:, 1:], dim=1)
        # # x = torch.mean(self.norm(x), dim=1)
        # x_n = self.pre_logits(x_n)
        # return x_n
        # v2: two different output
        # patch_feature = torch.mean(self.norm(x)[:, 1:], dim=1)
        # x = self.norm(x)[:, 0]
        # x = self.pre_logits(x)
        # return x, patch_feature
        # v3: return N patch embedding
        x = self.norm(x)
        # patch_features = self.norm(x)
        # x = self.norm(x)[:, 0]
        x = self.pre_logits(x)
    ...
    def forward(self, x, aug=False):
        x = self.forward_features(x, aug=aug)
        x = self.head(x)
