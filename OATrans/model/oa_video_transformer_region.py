...
class SpaceTimeTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 num_frames=8, time_init='rand', attention_style='frozen-in-time', modality_token=False, two_outputs=False):
        """
        Args:
        ...
        # object features embedding
        self.object_embed = nn.Linear(2054, embed_dim)
        ...
        self.region_norm = norm_layer(embed_dim)
    ...
    def forward_features(self, x):
        ...
        # remove the patch not contain the object
        # print(x.size(), patch_mask.size())
        # if mask:
        #     x = x * patch_mask.unsqueeze(2)
        BF = x.shape[0]
        ...
        # v1: extract region feature from last layer
        # for blk in self.blocks:
        #     x = blk(x, self.einops_from_space, self.einops_to_space, self.einops_from_time,
        #             self.einops_to_time,
        #             time_n=n, space_f=f)
        # cls_token = self.norm(x)[:, 0]
        # region_feature = self.norm(x)[:, 1:]
        # # x = self.pre_logits(x)
        # return cls_token, region_feature
        # v2: extract region feature from k-th layer
        K = 6
        count = 0
        for blk in self.blocks:
            x = blk(x, self.einops_from_space, self.einops_to_space, self.einops_from_time,
            ...
            count += 1
            if count == K:
                region_feature = self.region_norm(x)[:, 1:]
            # print(count)
        cls_token = self.norm(x)[:, 0]
        # x = self.pre_logits(x)
        return cls_token, region_feature
...
if __name__ == '__main__':
    x = torch.zeros([2, 1, 3, 224, 224])
    o = torch.zeros([2, 20, 2054])
    object_transformer = SpaceTimeObjectTransformer()
    object_transformer.eval()
    y = object_transformer(x, o)
    print(y.size())